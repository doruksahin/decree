...
def _spec(id: str, status: str, done: int, total: int, refs: tuple[str, ...] = ()) -> DocSummary:
    return DocSummary(id=id, type="spec", status=status, title=id, progress_done=done, progress_total=total, references=refs)
...
class TestDetectPhaseForChain:
    def test_implementation_phase(self):
        chain = Chain(prd=_prd("PRD-001", "approved"), adrs=(_adr("ADR-0001", "accepted", refs=("PRD-001",)),), specs=(_spec("SPEC-001", "draft", 5, 10, refs=("ADR-0001",)),))
        result = _detect_phase_for_chain(chain)
        assert result is not None
        phase, sug = result
        assert phase == Phase.IMPLEMENTATION
        assert sug.target_id == "SPEC-001"

    def test_completion_phase(self):
        chain = Chain(prd=_prd("PRD-001", "approved"), adrs=(_adr("ADR-0001", "accepted", refs=("PRD-001",)),), specs=(_spec("SPEC-001", "draft", 10, 10, refs=("ADR-0001",)),))
        result = _detect_phase_for_chain(chain)
        assert result is not None
        phase, sug = result
    ...
    def test_planning_phase(self):
        chain = Chain(prd=_prd("PRD-001", "approved"), adrs=(_adr("ADR-0001", "accepted", refs=("PRD-001",)),), specs=(_spec("SPEC-001", "draft", 0, 10, refs=("ADR-0001",)),))
        result = _detect_phase_for_chain(chain)
        assert result is not None
        phase, _ = result
    ...
    def test_technical_design_phase(self):
        chain = Chain(prd=_prd("PRD-001", "approved"), adrs=(_adr("ADR-0001", "accepted", refs=("PRD-001",)),), specs=())
        result = _detect_phase_for_chain(chain)
        assert result is not None
        phase, _ = result
    ...
    def test_architecture_decisions_phase(self):
        chain = Chain(prd=_prd("PRD-001", "approved"), adrs=(), specs=())
        result = _detect_phase_for_chain(chain)
        assert result is not None
        phase, _ = result
    ...
    def test_first_match_wins_implementation_over_design(self):
        ...
        chain = Chain(
            prd=_prd("PRD-001", "approved"),
            adrs=(_adr("ADR-0001", "accepted", refs=("PRD-001",)), _adr("ADR-0002", "accepted", refs=("PRD-001",))),
            specs=(_spec("SPEC-001", "draft", 5, 10, refs=("ADR-0001",)),),  # in-flight, refs only ADR-0001
        )
        result = _detect_phase_for_chain(chain)
        assert result is not None
        phase, _ = result
        # IMPLEMENTATION wins over the fact that ADR-0002 has no referencing SPEC.
        assert phase == Phase.IMPLEMENTATION

    def test_terminal_status_skipped(self):
        """An already-implemented SPEC at any progress doesn't trigger active phases."""
        chain = Chain(prd=_prd("PRD-001", "approved"), adrs=(_adr("ADR-0001", "accepted", refs=("PRD-001",)),), specs=(_spec("SPEC-001", "implemented", 10, 10, refs=("ADR-0001",)),))
        result = _detect_phase_for_chain(chain)
...
class TestDetectPhaseAggregation:
    ...
    def test_done_when_all_terminal(self):
        chain = Chain(prd=_prd("PRD-001", "implemented"), adrs=(_adr("ADR-0001", "accepted", refs=("PRD-001",)),), specs=(_spec("SPEC-001", "implemented", 10, 10, refs=("ADR-0001",)),))
    ...
    def test_implementation_outranks_architecture(self):
        ...
        chain1 = Chain(prd=_prd("PRD-001", "approved"), adrs=(_adr("ADR-0001", "accepted", refs=("PRD-001",)),), specs=(_spec("SPEC-001", "draft", 5, 10, refs=("ADR-0001",)),))
        chain2 = Chain(prd=_prd("PRD-002", "approved"), adrs=(), specs=())  # would be ARCHITECTURE_DECISIONS
    ...
    def test_dedup_same_spec_across_chains(self):
        ...
        shared_spec = _spec("SPEC-001", "draft", 5, 10, refs=("ADR-0001", "ADR-0002"))
        chain1 = Chain(prd=_prd("PRD-001", "approved"), adrs=(_adr("ADR-0001", "accepted", refs=("PRD-001",)),), specs=(shared_spec,))
        chain2 = Chain(prd=_prd("PRD-002", "approved"), adrs=(_adr("ADR-0002", "accepted", refs=("PRD-002",)),), specs=(shared_spec,))
        phase, sugs = _detect_phase({"doc_count": 5}, (chain1, chain2))
        assert phase == Phase.IMPLEMENTATION
        assert len(sugs) == 1
        assert sugs[0].target_id == "SPEC-001"
...
def prd_only_corpus(tmp_path: Path) -> Path:
    ...
    for sub in ("prd", "adr", "spec"):
        (tmp_path / "decree" / sub).mkdir(parents=True)
    (tmp_path / "decree" / "prd" / "001-test.md").write_text(_prd_doc(status="approved", title="Test PRD"))
    return tmp_path
...
def in_flight_corpus(tmp_path: Path) -> Path:
    ...
    for sub in ("prd", "adr", "spec"):
        (tmp_path / "decree" / sub).mkdir(parents=True)
    (tmp_path / "decree" / "prd" / "001-test.md").write_text(_prd_doc(status="approved", title="Test PRD"))
    (tmp_path / "decree" / "adr" / "0001-test.md").write_text(_adr_doc(status="accepted", title="Test ADR", references=["PRD-001"]))
    (tmp_path / "decree" / "spec" / "001-test.md").write_text(_spec_doc(status="draft", title="Test SPEC", references=["ADR-0001"], done=2, total=5))
    return tmp_path
...
def _spec_doc(*, status: str, title: str, done: int, total: int, references: list[str] | None = None) -> str:
    refs = f"references: [{', '.join(references)}]\n" if references else ""
    boxes = "\n".join([f"- [x] Item {i}" for i in range(done)] + [f"- [ ] Item {i+done}" for i in range(total - done)])
    return f"""---
status: {status}
date: 2026-05-12
{refs}---
...
class TestAssessE2E:
    ...
    def test_chains_built_correctly(self, monkeypatch, in_flight_corpus: Path):
        ...
        c = a.chains[0]
        assert c.prd is not None and c.prd.id == "PRD-001"
        assert len(c.adrs) == 1 and c.adrs[0].id == "ADR-0001"
        assert len(c.specs) == 1 and c.specs[0].id == "SPEC-001"
...
class TestRun:
    def test_exit_code_zero_on_healthy(self, monkeypatch, in_flight_corpus: Path, capsys):
        monkeypatch.chdir(in_flight_corpus)
        args = argparse.Namespace(json=False, quiet=False, project=None)
        rc = run(args)
        assert rc == 0

    def test_json_flag_produces_json(self, monkeypatch, in_flight_corpus: Path, capsys):
        monkeypatch.chdir(in_flight_corpus)
        args = argparse.Namespace(json=True, quiet=False, project=None)
        rc = run(args)
    ...
    def test_project_flag_resolves_path(self, monkeypatch, in_flight_corpus: Path, capsys, tmp_path):
        ...
        args = argparse.Namespace(json=False, quiet=False, project=str(in_flight_corpus))
        rc = run(args)
        out = capsys.readouterr().out
        assert "IMPLEMENTATION" in out
        assert rc == 0
