...
def test_supersede_symmetry(adr_env, capsys):
    (adr_env / "ADR-0001-old.md").write_text(
        "---\nstatus: superseded\ndate: 2026-04-01\nsuperseded-by: ADR-0002\n---\n\n" + VALID_BODY
    )
    (adr_env / "ADR-0002-new.md").write_text(
...
def test_collects_all_errors(adr_env, capsys):
    (adr_env / "ADR-0001-bad.md").write_text("---\nstatus: draft\ndate: 2026-04-02\n---\n\n# T\n")
    (adr_env / "ADR-0002-bad.md").write_text("---\nstatus: nope\ndate: 2026-04-02\n---\n\n# T\n")
    assert run(argparse.Namespace()) == 1
