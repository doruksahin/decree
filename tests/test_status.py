...
def test_invalid_transition(adr_env, capsys):
    ...
    result = run(argparse.Namespace(action="reject", adr_id="ADR-0001", target_id=None))
    assert result == 1
    assert "cannot transition" in capsys.readouterr().err

def test_terminal_status(adr_env, capsys):
    run(argparse.Namespace(action="reject", adr_id="ADR-0001", target_id=None))
    result = run(argparse.Namespace(action="accept", adr_id="ADR-0001", target_id=None))
    assert result == 1
