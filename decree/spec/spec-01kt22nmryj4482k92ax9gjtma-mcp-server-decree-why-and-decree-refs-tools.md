...
id: SPEC-01KT22NMRYJ4482K92AX9GJTMA
references:
...
Implements the first slice of PRD-01KT22NMRS4QGHSFDBZ858PP1T R5 — a Model Context Protocol (MCP) server exposing decree's existing query API as task-shaped tools. v1 ships two tools (`decree.why` and `decree.refs`) that wrap the library functions already shipped in SPEC-01KT22NMRXWCS5TK5VC1FT6JER. The remaining R5 tools (`stale`, `health`, `intent_review`) ship alongside their underlying feature SPECs:

- `decree.stale` and `decree.health` — added when SPEC-01KT22NMRYNFYM7EN80WS2HD6F ships (staleness + hotspots).
- `decree.intent_review` — added when SPEC-01KT22NMRYRZQ59EC88VJ5R0N6 ships (intent-review + migration).
...
PRD-01KT22NMRS4QGHSFDBZ858PP1T's dependency table reserved `mcp[cli]` (the official Python SDK with FastMCP merged in, MIT). This SPEC pulls it in. Tool definitions use the FastMCP decorator pattern:
...
Both tools reuse the helpers shipped in SPEC-01KT22NMRXWCS5TK5VC1FT6JER. No new query logic. The MCP server is a **thin protocol layer** over existing library functions.
...
- **No `stale` / `health` MCP tools** — they don't exist as library functions yet. SPEC-01KT22NMRYNFYM7EN80WS2HD6F adds them.
- **No `intent_review(diff)` MCP tool** — same, SPEC-01KT22NMRYRZQ59EC88VJ5R0N6 adds it.
...
- After SPEC-01KT22NMRYJ4482K92AX9GJTMA ships, register the decree MCP server with Claude Code. From a session, the agent should be able to call `decree.why("src/decree/index_db.py")` and get SPEC-01KT22NMRX176PCT00SKJ9G2AQ back. PM does this manual smoke once.
...
- PRD-01KT22NMRS4QGHSFDBZ858PP1T R5 — the requirement this SPEC partially implements (v1 = 2 of 5 tools).
- ADR-01KT22NMRV9CP14X5982JJH161 — Option C hybrid; the MCP server reads from the index.
- SPEC-01KT22NMRXWCS5TK5VC1FT6JER — the `why()` and `refs()` library functions this SPEC wraps.
- SPEC-01KT22NMRYNFYM7EN80WS2HD6F (future) — will add `stale` and `health` MCP tools.
- SPEC-01KT22NMRYRZQ59EC88VJ5R0N6 (future) — will add `intent_review` MCP tool.
