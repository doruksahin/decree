...
# SPEC-004 governs Field — Frontmatter and Lint Validation

## Overview

Implements PRD-003 R2 (file-level `governs:` frontmatter field on any configured type). Promotes `governs:` from a free-form raw-metadata key to a typed, validated, lint-checked frontmatter field. Symbol-level resolution (`path#symbol`) is parsed and stored but not validated against the working tree — that's the v2 backlog (tree-sitter / LSP).
...
```
decree/spec/042-foo.md: governs path does not exist: src/api/missing.py
```

Symbol-level entries (`path#symbol`) are validated only on the path part. The symbol is preserved on `governs` for future use by SPEC-005 query commands.
...
- Symbol-level validation. Entries like `src/foo.py#bar` have their path part validated but the `bar` symbol is not resolved. That's PRD-003 R2 v2 (tree-sitter / LSP).
...
- Migration tooling. PRD-003 R9 (`decree migrate governs --suggest`) ships in its own SPEC.
...
- After implementation, the implementer should add `governs: ["src/decree/index_db.py", "src/decree/commands/index_db_cli.py"]` to SPEC-003's frontmatter and `governs: ["src/decree/commands/ddd.py", "src/decree/commands/hook.py", "src/decree/commands/report.py"]` to SPEC-002. Then `decree lint` must pass and `decree index rebuild && decree index status` should show `governs > 0` rows.
...
- PRD-003 R2 — the requirement this SPEC implements.
- ADR-0002 — the architectural constraint (index reads from doc.meta.*).
- SPEC-003 — the IndexDB which already reads `governs` off raw_metadata and needs to be migrated to the typed field.
