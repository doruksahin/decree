...
# SPEC-008 Coherence Gates, Staleness, and Ungoverned Hotspots

## Overview

Implements PRD-003 R6 + R7 — the *health surface* of the decree corpus. Two thematic deliverables:

1. **Coherence gates (R6)** — opt-in `decree lint` rules that catch document states that aren't malformed (decree's existing lint already catches those) but are *incoherent*: a SPEC marked `implemented` with 60% checkboxes done, an ADR `accepted` 18 months ago with no SPEC referencing it, deferred-to-v2 items dragging a SPEC's progress below 100%. These are the LLM-authored-codebase failure modes that motivated PRD-003 in the first place.
...
Both ride on the SQLite index that SPEC-003 shipped and the `commits` table that SPEC-006 populates via git trailers. SPEC-008 adds **no new third-party dependencies** — it reuses `git log` (already wrapped in SPEC-006) and the existing `IndexDB` API.

Per SPEC-007's scoping decision, this SPEC also adds **two new MCP tools** (`decree.stale`, `decree.health`) wrapping the new CLI commands' library functions.
...
deferred_sections_separated = true       # gate 2 (uses SPEC-002's existing patterns)
...
A document whose status is configured terminal-success (e.g., `implemented` for SPEC, `accepted` for ADR — already detected by `is_terminal_success()` in SPEC-002) must have **100% of primary acceptance criteria checked**. Deferred-section ACs are excluded (using `_parse_checkboxes_by_section` from SPEC-002).

Error format:
```
decree/spec/001-foo.md: status 'implemented' but primary AC progress is 32/37 (86%). Check remaining items or move them to a deferred section.
```

This is the exact bug SPEC-001 has dogfooded since this work started. SPEC-008 ships the gate; SPEC-001 either gets remediated or its remaining items get moved to a deferred section.

#### Gate 2 — Deferred sections counted separately

Already half-implemented in SPEC-002's completion-report logic. This gate **promotes** the section-classification to a first-class lint rule: when a doc has both primary and deferred ACs, lint surfaces the split in its output (informational, not an error). It also rejects checkboxes nested inside fenced code blocks (the SPEC-002 example-report cosmetic bug — illustrations in code blocks shouldn't count). Configurable via `deferred_sections` patterns from decree.toml.

Output (informational; gate emits as warnings, exit-code 0):
```
decree/spec/002-bar.md: 3 deferred-section ACs separated from primary (counted independently).
decree/spec/002-bar.md: 3 checkboxes inside fenced code blocks (ignored — illustrations).
```

The "ignored — illustrations" handling means **primary AC progress no longer over-counts code-block examples**. This requires SPEC-002's `_parse_checkboxes_by_section` to be extended to skip checkboxes inside ` ```...``` ` fences. The change is contained to that one function and benefits SPEC-002's report generation as well.
...
Error format:
```
decree/prd/002-foo.md: status 'approved' for 12 days with no referencing ADR or SPEC. Stalled? (threshold: 30 days)
```
...
Stale decisions (governed files have churned without the decision being touched):
  ...
  SPEC-091   28 commits since 2026-03-15 on governs paths:
...
### MCP tool additions (extending SPEC-007's server)

Per SPEC-007's scoping decision, this SPEC adds **two MCP tools** to the existing FastMCP server in `src/decree/commands/mcp_server.py`:

- `decree.stale` — returns the stale-decisions section as a dict.
- `decree.health` — returns both stale + hotspots sections.

Same 5-section docstring quality bar from SPEC-007. Both tools reuse the library functions this SPEC ships (no new query logic in the MCP layer).
...
- **No `pydriller` dependency** — churn analysis uses `git log` shellouts (same pattern as SPEC-006's `sync_commits_from_git`).
...
- **No auto-suggested ADR drafts** — frontiers C.1 in `docs/market-analysis/research-frontiers.md`. PRD-004 territory.
...
- **Hotspot — high-churn file WITH governance**: `src/api/auth.py` with 50 commits, governed by SPEC-099 → not flagged.
...
- After SPEC-008 ships: run `decree health` against the decree corpus itself. Expectations:
  - SPEC-001 is `draft` not `implemented`, so gate 1 doesn't apply yet.
  - PRD-002 was `approved` then `implemented` quickly — gate 3 wouldn't flag it.
  - PRD-001 has been `approved` since 2026-04-05; if `unreferenced_active = true` for PRD type, it would only flag if no SPEC references it. SPEC-001 does. → no flag.
  - Ungoverned hotspots in decree itself: `src/decree/cli.py` is high-churn but governed (SPEC-002 and SPEC-005 both touch it via their governs). So likely clean.
- The PM enables the gates one at a time after SPEC-008 ships, using `decree migrate audit-coherence` (SPEC-009) for impact assessment before global enablement.
...
- [x] Both tools follow SPEC-007's 5-section docstring structure.
...
- [x] Run `decree health` against the decree project itself; record findings in the SPEC-008 completion report.
- [x] Enable gate 1 (terminal-status progress) on the SPEC type in decree.toml; confirm lint passes (or remediate findings).
- [x] SPEC-008's frontmatter declares `governs: ["src/decree/commands/health.py"]` after the file exists.
...
- [ ] Auto-suggested ADR drafts from ungoverned hotspots — research-frontiers C.1 / PRD-004.
- [ ] GitHub / Slack push notifications.
- [ ] Custom health checks via decree.toml plugin hooks.

## References

- PRD-003 R6 (coherence gates) + R7 (staleness + hotspots) — what this SPEC implements.
- ADR-0002 — Option C hybrid; staleness queries hit the index.
- SPEC-002 — `_parse_checkboxes_by_section` reused and extended (code-fence handling).
- SPEC-003 — index substrate (especially the `commits` table from SPEC-006).
- SPEC-006 — git log / git interpret-trailers shellout pattern reused for churn analysis.
- SPEC-007 — MCP server extended with `stale` and `health` tools.
