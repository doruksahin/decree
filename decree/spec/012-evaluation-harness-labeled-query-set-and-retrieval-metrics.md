...
# SPEC-012 Evaluation Harness — Labeled Query Set and Retrieval Metrics

## Overview

Implements PRD-004 R3 — the evaluation harness. The spine of PRD-004: nothing R1 (calibrated abstention) claims is falsifiable without a labeled query set + metric runner. This SPEC ships that infrastructure.
...
3. **Plugin interface** — `RetrievalMethod` protocol so PRD-003's keyword baseline + future PRD-004 methods (calibrated, hybrid, etc.) plug in with one signature.
4. **Frozen baseline** — current PRD-003 v1 keyword scoring is named `keyword-v1`, results snapshotted to `eval/baselines/keyword-v1.json` so future runs report delta against a fixed point.
...
PM commits to **option (a)** from PRD-004's open question:
...
- **No external corpus in v1.** When PRD-004 evolves a real benchmark, a future SPEC adds an open-source ADR corpus (CNCF, C4 model project, etc.). Decree's own dogfood is enough to make the harness real and proves the methodology.
...
- **No new retrieval methods beyond `keyword-v1`** — that's SPEC-013's job (calibrated abstention) and future SPECs (hybrid, GraphRAG).
...
- **No live LLM calls in CI** — same as SPEC-011. Tests use fixture corpora.
...
- SPEC-012's `governs:` declares `["src/decree/eval/", "src/decree/commands/eval.py"]` after the directory exists.
- PM runs `decree retrieval-eval` against the v1 query set (30 queries) and records results in the SPEC-012 completion report. Numbers go in the report; reviewers can scrutinise.
...
- PRD-004 R3 — the requirement this SPEC implements.
- SPEC-005 — `queries.why()` reused as the file_path retrieval engine.
- SPEC-003 — `decisions_fts` table reused for concept queries.
