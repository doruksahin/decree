...
Implements PRD-002 (DDD CLI Command and Proofshot Integration). Three deliverables ship in this SPEC:
...
3. **Completion report on terminal-status transition** — when a document transitions to a terminal-success status (e.g., `decree status SPEC-NNN implement`), decree writes a markdown report capturing the document chain, AC state, and timestamps.
...
- `1` — lint errors present, OR stale state detected per any R6 coherence gate that is enabled in `decree.toml`. (Until PRD-003 R6 ships, the staleness check is "approved PRD with no referencing ADR after N days" — implemented inline in this SPEC as a simple check, refactored later when R6 ships.)
...
| 1 — ARCHITECTURE DECISIONS | PRD exists, no ADR has `references: [PRD-NNN]` | `decree new adr "<title>"` referencing the PRD |
| 2 — TECHNICAL DESIGN | ADR `accepted`, no SPEC has `references: [ADR-NNNN]` | `decree new spec "<title>"` referencing the ADR |
...
| 5 — COMPLETION | SPEC at 100% checkboxes | `decree status SPEC-NNN implement` |
...
```json
{
  ...
  "chains": [
    {
      "prd": {"id": "PRD-001", "status": "approved", "title": "C4 Architecture Support"},
      "adrs": [{"id": "ADR-0001", "status": "accepted", "title": "Coupled C4 Module..."}],
      "specs": [{"id": "SPEC-001", "status": "draft", "progress_percent": 86, "title": "C4 Validation..."}]
    }
  ],
  "suggested_actions": [
    {"action": "complete_spec", "spec_id": "SPEC-001", "remaining_items": 5},
    {"action": "start_spec", "prd_id": "PRD-002"}
  ],
...
1. Detects whether the current directory has a `decree.toml` — if not, exits silently with code 0 (the hook is opt-in per project).
...
if [[ -z "$PROJECT_ROOT" ]]; then
  exit 0  # not a decree-enabled project; silent no-op
fi
...
```markdown
# SPEC-001 Completion Report

**Document**: `decree/spec/001-c4-validation-and-diagram-generation.md`
**Transitioned to `implemented` on**: 2026-05-15
**Total documents in chain**: 3 (PRD-001 ← ADR-0001 ← SPEC-001)

## Document chain

| Type | ID | Status | Title |
|---|---|---|---|
| PRD | PRD-001 | approved | C4 Architecture Support |
| ADR | ADR-0001 | accepted | Coupled C4 Module vs Plugin Architecture |
| SPEC | SPEC-001 | implemented | C4 Validation and Diagram Generation |
...
Sections are derived from the source document — primary ACs and deferred sections are split using the same logic that will eventually power PRD-003 R6's coherence gates (extracted now so it's reused later).
...
- Does **not** depend on PRD-003 (SQLite index, `governs:` field). All reads use existing frontmatter parsing.
- Does **not** generate visual artifacts (Mermaid diagrams, screenshots, etc.). PRD-002 mentions "proofshot" — for decree's CLI scope, the proofshot equivalent is the textual completion report. Visual verification is a v2 add-on.
- Does **not** auto-transition statuses. The user still runs `decree status SPEC-NNN implement` manually; the report is a side-effect of *user-initiated* transitions.
- Does **not** retroactively generate reports for documents already in terminal-success state (e.g., SPEC-001 if it ever gets to `implemented`). A separate `decree report regenerate <id>` command can be added in v2 if needed.
...
- Run `decree ddd` against decree's own corpus (4 PRDs, 2 ADRs, 2 SPECs at SPEC-002's draft state) and confirm it reports Phase IMPLEMENTATION pointing at SPEC-002 and SPEC-001's remaining items. Manual smoke-check, not automated.

### Real-corpus test (the actual integration test)

- Run `decree ddd --json` against jira-task-to-md's 167-document corpus. Assert the command completes in <500ms (with 100+ docs, the in-process implementation must not pathologically reparse). Assert the JSON output validates against the schema. This is the same corpus that PRD-003 R9 migration will be validated against — exercising decree against it now de-risks both SPECs.
...
- PRD-002 — DDD CLI Command and Proofshot Integration. The three requirements (R1 CLI, R2 proofshot, R3 stop hook) map 1:1 to the three components of this SPEC.
...
- SPEC-001 — uses the same "Files touched" + "v1 ACs" + "What this does NOT do" structure for consistency. The deferred-sections pattern documented here is what PRD-003 R6 will eventually formalize as a coherence gate.
