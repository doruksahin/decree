...
id: SPEC-01KT22NMS0VWCTYPFPHP8M8V36
references:
...
Implements PRD-01KT22NMRSXYT95XE808VD8EV4 R1 — *calibrated abstention*. Decree retrieval today returns the best-of-K result regardless of how weak the match is. This SPEC adds the principled "no governance found" response: a confidence-gated layer that abstains when no candidate is strong enough, with the threshold *calibrated* (not heuristic) against the SPEC-01KT22NMRZXE5C42F6Z0ZY559A query set.
...
2. **Conformal calibration** via the `crepes` library (MIT) over the SPEC-01KT22NMRZXE5C42F6Z0ZY559A query set. Output: persisted `prediction_set_threshold` per coverage target (default 90% precision among returned answers). Tunable via `--target-precision` flag during calibration.
3. **New retrieval method `keyword-v1-calibrated`** registered with the SPEC-01KT22NMRZXE5C42F6Z0ZY559A eval harness; SPEC-01KT22NMS0KTWGNKB36RR7K0JR will compose with this. CLI surface: `decree why --with-abstention` opts user-facing queries into the calibrated layer; default behavior unchanged (back-compat).
...
| Gate | Inline (Repowise replica) | New (SPEC-01KT22NMS0VWCTYPFPHP8M8V36) | Signal |
|---|---|---|---|
...
| **recency** | | ✓ | `1 - (days_since_doc_date / 540)` clamped to [0, 1]. 540 days ≈ 18 months → 0 (per PRD-01KT22NMRSXYT95XE808VD8EV4 R1). |
...
Calibration flow:
1. Load SPEC-01KT22NMRZXE5C42F6Z0ZY559A query set. Split into calibration (60%) and test (40%) sets (deterministic shuffle with seed).
...
Registered via SPEC-01KT22NMRZXE5C42F6Z0ZY559A's `RetrievalMethod` Protocol. Adds three behaviors atop `keyword-v1`:
...
no governance found (composite confidence 0.34; threshold 0.62)
  ...
  closest non-abstaining hit: SPEC-<ULID> (would have been returned without --with-abstention)
```
...
- **No hybrid retrieval** — BM25 + dense + structural is research-frontiers A.1; the ranking gaps SPEC-01KT22NMRZXE5C42F6Z0ZY559A surfaced (umbrella-PRD beats implementing-SPEC) require a different SPEC.
...
- **Real corpus dogfood**: run `decree retrieval-eval --calibrate` on `eval/queries.yaml`. Then run regular eval comparing `keyword-v1` (baseline) and `keyword-v1-calibrated`. Capture coverage-risk numbers in the SPEC-01KT22NMS0VWCTYPFPHP8M8V36 completion report.

### Dogfood

- SPEC-01KT22NMS0VWCTYPFPHP8M8V36's `governs:` declares the new files after they exist.
- Run `decree why src/api/totally-not-a-thing.py --with-abstention` → expect abstention.
- Run `decree why src/decree/index_db.py --with-abstention` → expect SPEC-01KT22NMRX176PCT00SKJ9G2AQ.
...
- PRD-01KT22NMRSXYT95XE808VD8EV4 R1 — what this SPEC implements.
- SPEC-01KT22NMRZXE5C42F6Z0ZY559A — eval harness this SPEC plugs into; query set used for calibration.
- SPEC-01KT22NMRXWCS5TK5VC1FT6JER — `queries.why()` / `queries.refs()` extended with `with_abstention`.
