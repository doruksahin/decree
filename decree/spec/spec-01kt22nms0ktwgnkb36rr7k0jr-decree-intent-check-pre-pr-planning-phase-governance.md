...
Implements PRD-01KT22NMRSXYT95XE808VD8EV4 R2 — **pre-PR intent_check**. The post-code counterpart (`decree intent-review`) was shipped in SPEC-01KT22NMRYRZQ59EC88VJ5R0N6. This SPEC adds the *planning-phase* counterpart: an LLM agent says "I'm about to implement X" and `decree.intent_check(plan, planned_files)` returns the governance map *before* any code is written.

This is the entire.io thesis at decree's surface: shift decree from passive record-keeping to active influence on what gets built. After SPEC-01KT22NMS0KTWGNKB36RR7K0JR lands, **PRD-01KT22NMRSXYT95XE808VD8EV4 v1 closes**.

The implementation is mostly composition of existing components — same trick that worked for SPEC-01KT22NMRYRZQ59EC88VJ5R0N6:
- `queries.why()` / `queries.refs()` from SPEC-01KT22NMRXWCS5TK5VC1FT6JER
- `health.stale_decisions()` from SPEC-01KT22NMRYNFYM7EN80WS2HD6F
- Calibrated abstention from SPEC-01KT22NMS0VWCTYPFPHP8M8V36 (so intent_check inherits the trust surface)
- Optional LLM-judged semantic conflict detection via `litellm` (already in deps from SPEC-01KT22NMRZZ0ZZ0DQ4N0SJPN9S) — the genuinely-new bit
...
- `--with-abstention` — route governance lookups through SPEC-01KT22NMS0VWCTYPFPHP8M8V36's calibrated method.
- `--target-precision P` — calibration target (same semantics as SPEC-01KT22NMS0VWCTYPFPHP8M8V36).
...
- `--model MODEL` — litellm model string (same resolution chain as SPEC-01KT22NMRZZ0ZZ0DQ4N0SJPN9S).
...
class IntentCheckReport:
    ...
    governing_decisions: tuple[GoverningSnapshot, ...]   # reused from SPEC-01KT22NMRYRZQ59EC88VJ5R0N6
    stale_governance: tuple[dict, ...]                   # reused from SPEC-01KT22NMRYRZQ59EC88VJ5R0N6
...
Differences from SPEC-01KT22NMRYRZQ59EC88VJ5R0N6's `IntentReport`:
...
- **`recommended_actions` new verbs**:
  ...
  - Plus reuse of SPEC-01KT22NMRYRZQ59EC88VJ5R0N6 verbs: `update_decision`, `check_ac`, `add_governance`.

### LLM-judged conflict detection (`--judge-conflicts`)

For each structural conflict from the index (two decisions sharing a `governs:` path), call an LLM with this prompt (added to `src/decree/migrate_prompts.py` alongside SPEC-01KT22NMRZZ0ZZ0DQ4N0SJPN9S's prompt):
...
Uses `litellm.completion()` with the same model-resolution chain as SPEC-01KT22NMRZZ0ZZ0DQ4N0SJPN9S. Failures fall back to structural-only (the conflict still surfaces, just without semantic verdict).
...
2. From the id set: pull unchecked ACs (same SQL as SPEC-01KT22NMRYRZQ59EC88VJ5R0N6).
3. Call `health.stale_decisions(db, project_root, threshold_commits)`; intersect with governing ids → `stale_governance`.
4. Structural conflict detection (same SQL as SPEC-01KT22NMRYRZQ59EC88VJ5R0N6).
...
The 6th tool. Same 5-section docstring quality bar from SPEC-01KT22NMRYJ4482K92AX9GJTMA.
...
def intent_check(
    ...
    Args:
        ...
        with_abstention: Route governance lookups through calibrated abstention
            (SPEC-01KT22NMS0VWCTYPFPHP8M8V36). Recommended for agent loops that will trust the output.
    ...
    When not to call:
        ...
        - After code is written — that's `intent_review` (SPEC-01KT22NMRYRZQ59EC88VJ5R0N6), not this.
...
- **Create**: `src/decree/commands/intent_check.py` — library function + CLI handler + dataclasses (reuse SPEC-01KT22NMRYRZQ59EC88VJ5R0N6's dataclasses where possible).
...
- **End-to-end against decree corpus**: `decree intent-check --plan "Add staleness threshold config" --files src/decree/commands/health.py`. Should return SPEC-01KT22NMRYNFYM7EN80WS2HD6F as governing.
- **End-to-end JSON**: parsable, has expected keys.

### Dogfood

- SPEC-01KT22NMS0KTWGNKB36RR7K0JR's `governs:` declares `src/decree/commands/intent_check.py` after creation.
...
- PRD-01KT22NMRSXYT95XE808VD8EV4 R2 — what this SPEC implements.
- SPEC-01KT22NMRYRZQ59EC88VJ5R0N6 — post-code `intent_review`; this SPEC mirrors its shape for the pre-code phase.
- SPEC-01KT22NMS0VWCTYPFPHP8M8V36 — calibrated abstention used here when `--with-abstention` is set.
- SPEC-01KT22NMRZZ0ZZ0DQ4N0SJPN9S — litellm + `_parse_llm_json` pattern reused for the LLM judge.
