...
A third concern is dogfood timing. SPEC-01KT22NMRWENYKC3MGRA50M7GE is still in `draft` at 86% (with v2 backlog items in its checkbox list — exactly the kind of coherence issue PRD-01KT22NMRS4QGHSFDBZ858PP1T R6 is meant to fix). PRD-01KT22NMRR0BX7KBF0F0N5ER6Z is `approved` but at 0%. Migrating existing commands while in-flight work is open is the textbook setup for a stalled migration.
...
- PRD-01KT22NMRS4QGHSFDBZ858PP1T requires sub-100ms `decree why <path>` on a 100+ document corpus
...
- Existing commands (`lint`, `progress`, `ddd`, `new`, `status`) work today; they should not regress while v1 of PRD-01KT22NMRS4QGHSFDBZ858PP1T ships
...
- Bad: SPEC-01KT22NMRWENYKC3MGRA50M7GE and PRD-01KT22NMRR0BX7KBF0F0N5ER6Z work has to either pause or land *after* the migration, which means more documents to migrate when the dust settles
...
Frontmatter remains the source of truth, including for existing commands. Add a SQLite index as a *derived read-cache*, refreshed by `decree lint` (and a pre-commit hook). Only the new PRD-01KT22NMRS4QGHSFDBZ858PP1T commands (`why`, `refs`, `stale`, `health`) and the MCP server read from the index. Existing commands (`lint`, `progress`, `ddd`, `new`, `status`) are not migrated in v1 — they continue to read frontmatter.
...
- Good: existing commands unchanged → SPEC-01KT22NMRWENYKC3MGRA50M7GE and PRD-01KT22NMRR0BX7KBF0F0N5ER6Z work proceeds in parallel without merge conflicts
...
The decisive factor is dogfood timing. Option A's "rebuild everything" path is architecturally cleaner, but PRD-01KT22NMRS4QGHSFDBZ858PP1T has to ship while SPEC-01KT22NMRWENYKC3MGRA50M7GE and PRD-01KT22NMRR0BX7KBF0F0N5ER6Z are open, and a simultaneous migration of every existing command is the worst possible context for that. Option C ships the new query surface in v1 without touching the existing commands' read paths — they remain on frontmatter, they remain provably correct under the existing test suite.
...
Option B is rejected because it makes the MCP server a leaky wrapper. PRD-01KT22NMRS4QGHSFDBZ858PP1T's success criteria require sub-100ms `why` latency and a stable MCP API; Option B cannot meet either without doing most of Option A's work anyway.
