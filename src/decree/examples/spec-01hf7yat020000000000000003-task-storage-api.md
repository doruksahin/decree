---
id: SPEC-01HF7YAT020000000000000003
status: approved
date: 2026-01-20
references: [PRD-01HF7YAT000000000000000001, ADR-01HF7YAT010000000000000002]
---

# SPEC-01HF7YAT020000000000000003 Task Storage API

## Overview

Design the storage layer for the task CLI, implementing the SQLite backend chosen in ADR-01HF7YAT010000000000000002 to satisfy the persistence and query requirements from PRD-01HF7YAT000000000000000001. This spec defines the schema, the Python API, and the error handling contract.
...
Every method that writes runs inside a `with self.conn:` transaction block — SQLite commits on exit, rolls back on exception. This is the crash-safety guarantee from ADR-01KT22NMRV7GMAXKWSBEEN68KE.
...
- Performance: 10,000 tasks, `list(priority="high")` < 100ms (from ADR-01KT22NMRV7GMAXKWSBEEN68KE validation criteria)
