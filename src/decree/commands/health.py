"""SPEC-008 — `decree health` / `decree stale` command.

Surfaces the *data-push* health signals described in PRD-003 R7:
...
pattern as SPEC-006's `IndexDB.sync_commits_from_git`) joined against
...
functions reused by SPEC-008's MCP tools.
"""
...
def ungoverned_hotspots(
    ...
    for path, count in churn.items():
        ...
        findings.append(
            UngovernedHotspot(
                path=path, commit_count=count, since_days=threshold_days
            )
        )
...
def health(
    ...
    return HealthReport(
        ...
        ungoverned_hotspots=tuple(
            ungoverned_hotspots(db, project_root, threshold_commits, threshold_days)
        ),
...
def _format_human(report: HealthReport) -> str:
    ...
    if report.stale_decisions:
        ...
        for sd in report.stale_decisions:
            when = (
                datetime.fromtimestamp(sd.last_touched_ts, tz=timezone.utc)
                .date()
                .isoformat()
            ...
            lines.append(
                f"  {sd.decision_id}   {sd.churn_count} commits since {when} on governs paths:"
            )
    ...
    if report.ungoverned_hotspots:
        ...
        for h in report.ungoverned_hotspots:
            lines.append(
                f"  {h.path}  {h.commit_count} commits in last {h.since_days} days "
                f"— no governing decision"
            )
        lines.append("")
    else:
    ...
    lines.append(
        f"Thresholds: --threshold-commits={report.threshold_commits} "
...
def _resolve_thresholds(args: argparse.Namespace) -> tuple[int, int]:
    ...
    try:
        ...
        cfg = None
    t_commits = (
        args.threshold_commits
        if args.threshold_commits is not None
        else (cfg.threshold_commits if cfg else 10)
    )
    t_days = (
        args.threshold_days
        if args.threshold_days is not None
        else (cfg.threshold_days if cfg else 30)
    )
