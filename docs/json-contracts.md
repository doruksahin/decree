...
| `intent-check --json` | `plan`, `planned_files[]`, `governing_decisions[]`, `conflicts[]`, `live_conflicts[]`, `governs_gaps[]`, `stale_governance[]`, `recommended_actions[]`, `under_decision`, `under_error` | `0` clean; **`1` with full JSON** on conflicts/stale/live-conflicts; `2` on error |
